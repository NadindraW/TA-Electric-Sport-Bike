...
uint8_t show_speed(){
    ...
    uint8_t val_speed = mVotolRPM / 46;
 ...
 */
uint8_t show_battr() {
    // using EV battery V1 raw data in 630 - 830 to translate in 0 - 100
    //uint16_t val_battr = ((mVotolVoltage > 630) ? (mVotolVoltage-630)/2 : 0); 

    // using EV battery V2 raw data in 620 - 680 to translate in 0 - 100
    uint16_t val_battr = ((mVotolVoltage > 620) ? (mVotolVoltage*10)/68  : 0);
    ...
    float val_v = mVotolVoltage * 0.1;
    Serial.print("obj_v.txt=\"");
    Serial.print(val_v);
    Serial.print("\"\xFF\xFF\xFF");
...
uint8_t show_state() {
    ...
    if (mVotolRPM > 100) { 
        mVotolState = 2;  Serial.print("\"D\""); }          // 0x02 - 0b00000010 (DRIVE)   
    else if (bitRead(inDataVotol[20], 3) == 1){ 
        mVotolState = 3; Serial.print("\"P\""); }           // 0x03 - 0b00000011 (PARKING)
    else { mVotolState = 1; Serial.print("\"N\""); }        // 0x01 - 0b00000001 (NORMAL) 
    Serial.print("\xFF\xFF\xFF");
    return mVotolState;
}
...
uint8_t show_modes() {
    ...
    if (mVotolMode == 1) Serial.print("\"ECONOMIC\"");      // 0x01 - 0b00000001 (ECON)
    else if (mVotolMode == 2) Serial.print("\"COMFORT\"");  // 0x02 - 0b00000010 (COMF)
    else if (mVotolMode == 3) Serial.print("\"SPORT\"");    // 0x03 - 0b00000011 (SPRT)
    Serial.print("\xFF\xFF\xFF");
    return mVotolMode;
}
