...
#define BEAMS_PIN 27     // V1:27  V2: 4  V3:35
...
#define I2S_GAN_PIN 23  // V1:23  V2:27  V3:27 
...
bool val_i2s;
uint8_t inData_votol[30];  // (Feedback) Data received by VOTOL 
uint8_t val_state, val_sein;
uint16_t voltage_vtl, ampere_vtl, rpm_vtl, contemp_vtl;
uint16_t exttemp_vtl, mode_vtl = 3, state_vtl = 3;
...
void setup() {
    ...
    // i2s_bt_media_setup();                        // Serial Bluetooth Speaker
...
void loop() {
    ...
    // if(val_state != 3 && val_i2s == true){
    //   delay(1000); 
    //   // vTaskSuspend(media_task_handle);
    //   i2s_bt_media_end();
    // } else if (val_state == 3 && val_i2s == false){
    //   delay(1000); 
